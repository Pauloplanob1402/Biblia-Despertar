...
import { 
  ...
  AlertCircle, ChevronRight, Terminal, FileText, Check, Code, MessageSquare, Award, Smartphone, Share2, BookOpen,
  Home, Users
} from 'lucide-react';
...
export default function App() {
  const [activeSection, setActiveSection] = useState<'home' | 'bible' | 'devotionals' | 'profiles' | 'mesas' | 'ebooks' | 'profile' | 'respiro' | 'testemunhas' | 'primitiva' | 'apps' | 'mural' | 'apoiar' | 'palavra_hub' | 'comunidade_hub' | 'primitiva_mural' | 'primitiva_visao' | 'primitiva_centelhas'>('home');
    ...
    <div className="min-h-screen bg-stone-50 text-stone-900 flex flex-col md:flex-row antialiased selection:bg-[#C08261]/20">
      ...
      <header className="md:hidden w-full bg-white border-b border-stone-200/55 flex items-center justify-between p-4 sticky top-0 z-30 shrink-0 select-none">
        ...
        <button 
          ...
          className="hidden md:flex px-3.5 py-1.5 text-xs font-mono font-bold tracking-wider uppercase text-[#C08261] bg-stone-50 hover:bg-stone-100 rounded-xl border border-stone-200/50 transition items-center space-x-1"
        >
          <span>Menu ☰</span>
        </button>
      </header>
            ...
            >
              ...
              {currentUser ? (
                <div className="p-5 border-b border-stone-100 bg-stone-50/50 flex items-center justify-between text-left">
                  <div className="flex items-center space-x-3 truncate">
                    <div className="w-10 h-10 rounded-xl bg-orange-100/80 flex items-center justify-center text-xl shadow-xs shrink-0 select-none overflow-hidden">
              ...
              <nav className="flex-1 p-4 space-y-1 overflow-y-auto w-full">
                ...
                <button
                  ...
                  onClick={() => { setActiveSection('palavra_hub'); setSelectedDevotional(null); setIsMobileMenuOpen(false); }}
                  className={`w-full flex items-center px-3 py-3 rounded-xl text-left text-sm font-semibold transition ${
                    ['palavra_hub','devotionals','bible','respiro','ebooks'].includes(activeSection) ? 'bg-[#C08261]/10 text-[#C08261]' : 'text-stone-700 hover:bg-stone-50'
                  }`}
                >
                  <span className="flex items-center space-x-3">
                    <Book size={15} />
                ...
                <div className="ml-7 space-y-0.5 pb-1">
                  {[
                    { id: 'bible' as const, label: '📖 Bíblia Sagrada' },
                    { id: 'devotionals' as const, label: '🕯️ Reflexões & Meditação' },
                    { id: 'respiro' as const, label: '🌬️ Respiro do Silêncio' },
                    { id: 'ebooks' as const, label: '📚 E-Books & Livros' },
                  ].map(item => (
                    <button key={item.id} onClick={() => { setActiveSection(item.id); setSelectedDevotional(null); setIsMobileMenuOpen(false); }}
                ...
                <button
                  ...
                  onClick={() => { setActiveSection('comunidade_hub'); setSelectedDevotional(null); setIsMobileMenuOpen(false); }}
                  className={`w-full flex items-center px-3 py-3 rounded-xl text-left text-sm font-semibold transition ${
                    ['comunidade_hub','mural','testemunhas','primitiva','primitiva_mural','primitiva_visao','primitiva_centelhas','profiles'].includes(activeSection) ? 'bg-[#C08261]/10 text-[#C08261]' : 'text-stone-700 hover:bg-stone-50'
                  }`}
                >
                  <span className="flex items-center space-x-3">
                    <Compass size={15} />
                    <span>Comunhão & Fé</span>
                  </span>
                </button>
                <div className="ml-7 space-y-0.5 pb-1">
                  {[
                    { id: 'primitiva_mural' as const,    label: '🌱 O Mural Vivo' },
                    { id: 'mural' as const,               label: '🕊️ Mural de Oração' },
                    { id: 'testemunhas' as const,         label: '🌟 Nuvem de Testemunhas' },
                    { id: 'primitiva_visao' as const,     label: '📖 Manifesto & Apoio' },
                    { id: 'primitiva_centelhas' as const, label: '💡 Centelhas de Fé' },
                    { id: 'profiles' as const,            label: '🧭 Jornadas de Fé' },
                  ].map(item => (
                    <button key={item.id} onClick={() => { setActiveSection(item.id); setSelectedDevotional(null); setIsMobileMenuOpen(false); }}
                      className={`w-full flex items-center px-3 py-1.5 rounded-lg text-left text-xs font-medium transition ${
                        activeSection === item.id ? 'text-[#C08261] font-semibold' : 'text-stone-500 hover:text-stone-700'
      ...
      <aside className="hidden md:flex w-72 bg-white border-r border-stone-200/55 flex-col shrink-0 h-screen sticky top-0 z-30">
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
          ...
          {/* 2. PALAVRA DE DEUS — Hub + Accordion sub-links */}
          <button
            id="nav-palavra"
            onClick={() => { setActiveSection('palavra_hub'); setSelectedDevotional(null); }}
            className={`w-full flex items-center justify-between px-3 py-3 rounded-xl text-left tracking-wide text-sm font-semibold transition ${
              ['palavra_hub','devotionals','bible','respiro','ebooks'].includes(activeSection) ? 'bg-[#C08261]/10 text-[#C08261]' : 'text-stone-700 hover:bg-stone-50'
            }`}
          >
            ...
            <ChevronRight size={12} className={`transition-transform ${['palavra_hub','devotionals','bible','respiro','ebooks'].includes(activeSection) ? 'rotate-90' : ''}`} />
          </button>
          {/* Accordion Palavra */}
          {['palavra_hub','devotionals','bible','respiro','ebooks'].includes(activeSection) && (
            <div className="ml-7 space-y-0.5 pb-1">
              {[
                { id: 'bible' as const,      label: '📖 Bíblia Sagrada',       desc: 'Leitura e favoritos' },
                { id: 'devotionals' as const, label: '🕯️ Reflexões & Meditação', desc: 'Devocional diário' },
                { id: 'respiro' as const,    label: '🌬️ Respiro do Silêncio',   desc: 'Respiração guiada' },
                { id: 'ebooks' as const,     label: '📚 E-Books & Livros',      desc: 'Obras do Despertar' },
              ].map(item => (
                <button key={item.id} onClick={() => { setActiveSection(item.id); setSelectedDevotional(null); }}
                  className={`w-full flex flex-col px-3 py-2 rounded-lg text-left transition ${
                    activeSection === item.id ? 'bg-[#C08261]/8 text-[#C08261]' : 'text-stone-500 hover:text-stone-700 hover:bg-stone-50/50'
                  }`}>
                  <span className={`text-xs font-semibold ${activeSection === item.id ? 'text-[#C08261]' : ''}`}>{item.label}</span>
                  <span className="text-[10px] text-stone-400 font-sans">{item.desc}</span>
                </button>
              ))}
            </div>
          )}

          {/* 3. COMUNHÃO & FÉ — Hub + Accordion sub-links desagregados */}
          <button
            id="nav-comunidade"
            onClick={() => { setActiveSection('comunidade_hub'); setSelectedDevotional(null); }}
            className={`w-full flex items-center justify-between px-3 py-3 rounded-xl text-left tracking-wide text-sm font-semibold transition ${
              ['comunidade_hub','mural','testemunhas','primitiva','primitiva_mural','primitiva_visao','primitiva_centelhas','profiles'].includes(activeSection) ? 'bg-[#C08261]/10 text-[#C08261]' : 'text-stone-700 hover:bg-stone-50'
            }`}
          >
            ...
            <ChevronRight size={12} className={`transition-transform ${['comunidade_hub','mural','testemunhas','primitiva','primitiva_mural','primitiva_visao','primitiva_centelhas','profiles'].includes(activeSection) ? 'rotate-90' : ''}`} />
          </button>
          {/* Accordion Comunidade */}
          {['comunidade_hub','mural','testemunhas','primitiva','primitiva_mural','primitiva_visao','primitiva_centelhas','profiles'].includes(activeSection) && (
            <div className="ml-7 space-y-0.5 pb-1">
              {[
                { id: 'primitiva_mural' as const,    label: '🌱 O Mural Vivo',             desc: 'Amor prático em ação' },
                { id: 'mural' as const,               label: '🕊️ Mural de Oração',          desc: 'Intercessão coletiva' },
                { id: 'testemunhas' as const,         label: '🌟 Nuvem de Testemunhas',     desc: 'Histórias de fé' },
                { id: 'profiles' as const,            label: '🧭 Jornadas de Fé',           desc: 'Identidades espirituais' },
                { id: 'primitiva_visao' as const,     label: '📖 Manifesto & Apoio',        desc: 'Visão e doações' },
                { id: 'primitiva_centelhas' as const, label: '💡 Centelhas de Fé Ativa',    desc: 'Co-criação comunitária' },
              ].map(item => (
                <button key={item.id} onClick={() => { setActiveSection(item.id); setSelectedDevotional(null); }}
                  className={`w-full flex flex-col px-3 py-2 rounded-lg text-left transition ${
                    activeSection === item.id ? 'bg-[#C08261]/8 text-[#C08261]' : 'text-stone-500 hover:text-stone-700 hover:bg-stone-50/50'
          ...
          {/* 6. COMO APOIAR — destaque para os ebooks */}
          <button
            id="nav-apoiar"
            onClick={() => { setActiveSection('apoiar'); setSelectedDevotional(null); }}
            className={`w-full flex items-center justify-between px-3 py-3 rounded-xl text-left tracking-wide text-sm font-semibold border transition ${
              activeSection === 'apoiar'
                ? 'bg-amber-50 border-amber-300 text-amber-800'
                : 'bg-amber-50/60 border-amber-200/70 text-amber-900 hover:bg-amber-50 hover:border-amber-300'
            }`}
          >
            <span className="flex items-center space-x-3">
            ...
            <span className="text-[9px] bg-amber-500 text-white px-2 py-0.5 rounded-full font-mono uppercase font-bold tracking-wide">
              📚 Livros
            </span>
          </button>
      ...
      <main className="flex-1 overflow-y-auto px-6 py-8 md:p-12 max-w-7xl mx-auto w-full relative pb-20 md:pb-12">

        {/* ── BARRA DE RETORNO DINÂMICO — mobile inner back bar ─────────────────
            Exibida quando o usuário entra em sub-seção de um Hub.
            Permite retornar ao Hub sem usar o menu principal.
        ────────────────────────────────────────────────────────────────────── */}
        {(['bible','devotionals','respiro','ebooks'].includes(activeSection) && !selectedDevotional) && (
          <div className="md:hidden flex items-center mb-4">
            <button
              onClick={() => setActiveSection('palavra_hub')}
              className="flex items-center space-x-1.5 text-xs font-mono font-semibold text-[#C08261] bg-[#C08261]/10 border border-[#C08261]/25 px-3 py-2 rounded-xl transition hover:bg-[#C08261]/15"
            >
              <ArrowLeft size={13} />
              <span>Voltar para Palavra</span>
            </button>
          </div>
        )}
        {(['mural','testemunhas','primitiva','primitiva_mural','primitiva_visao','primitiva_centelhas','profiles'].includes(activeSection) && !selectedDevotional) && (
          <div className="md:hidden flex items-center mb-4">
            <button
              onClick={() => setActiveSection('comunidade_hub')}
              className="flex items-center space-x-1.5 text-xs font-mono font-semibold text-[#C08261] bg-[#C08261]/10 border border-[#C08261]/25 px-3 py-2 rounded-xl transition hover:bg-[#C08261]/15"
            >
              <ArrowLeft size={13} />
        ...
        <AnimatePresence mode="wait">
          ...
          {activeSection === 'profile' && !selectedDevotional && (
            <motion.div
              ...
              className="space-y-8 text-left"
            >
              <div className="border-b border-stone-105 pb-6 space-y-4">
                {currentUser && (
                  <div className="flex flex-col md:flex-row items-center md:items-start gap-5">
                    <AvatarUpload
                      currentAvatar={userProfile?.photoURL || userProfile?.avatarEmoji || '🕊️'}
                      onUploadSuccess={(url) => {
                        setUserProfile((prev: any) => ({ ...prev, photoURL: url }));
                      }}
                    />
                    <div className="text-center md:text-left space-y-1">
                      <span className="text-xs md:text-sm font-mono uppercase tracking-widest text-[#C08261] font-bold block">
                        Minha caminhada de Fé
                      </span>
                      <h3 className="font-serif text-3xl md:text-4xl font-light text-stone-850 tracking-tight leading-tight">
                        {userProfile?.name || currentUser.displayName || 'Peregrino'}
                      </h3>
                      <p className="text-stone-500 text-sm leading-relaxed font-sans max-w-xl">
                        Acompanhe seus tempos de quietude, notas e versículos favoritos.
                      </p>
                    </div>
                  </div>
                )}
          ...
          {/* HUB DA PALAVRA — Gateway centralizado para sub-seções da Palavra */}
          {activeSection === 'palavra_hub' && !selectedDevotional && (
            <motion.div
              key="palavra_hub"
              initial={{ opacity: 0, y: 15 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              <div className="space-y-2">
                <span className="text-[10px] font-mono uppercase tracking-widest text-[#C08261] font-bold">Hub da Palavra</span>
                <h2 className="font-serif text-3xl md:text-4xl font-light text-stone-850 tracking-tight leading-tight">A Mesa da <span className="text-[#C08261] font-semibold">Palavra Viva</span></h2>
                <p className="text-stone-500 text-sm max-w-xl font-sans leading-relaxed">
                  Escolha como deseja se encontrar com Deus hoje. Cada portal abaixo é um altar preparado especialmente para o seu momento.
                </p>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                {[
                  { id: 'bible' as const, icon: '📖', label: 'Bíblia Sagrada', desc: 'Navegue pelos 66 livros da Palavra. Sublinhe versículos faróis e guarde passagens no seu coração.', accent: '#C08261' },
                  { id: 'devotionals' as const, icon: '🕯️', label: 'Reflexões & Meditação', desc: 'Encontros devocionais diários que traduzem a Bíblia para a linguagem do seu dia a dia real.', accent: '#C08261' },
                  { id: 'respiro' as const, icon: '🌬️', label: 'Respiro do Silêncio', desc: 'Exercícios de respiração guiada para aquietar o coração em 4 segundos. Sem cadastro.', accent: '#5B8A6B' },
                  { id: 'ebooks' as const, icon: '📚', label: 'E-Books & Livros', desc: 'Obras do Despertar para levar a jornada além do devocional. Leitura transformadora em capítulos.', accent: '#8B6914' },
                ].map(item => (
                  <button
                    key={item.id}
                    onClick={() => setActiveSection(item.id)}
                    className="group relative overflow-hidden bg-white border border-stone-200/60 rounded-3xl p-7 text-left hover:border-[#C08261]/40 hover:shadow-lg transition-all duration-300 flex flex-col space-y-4"
                  >
                    <div className="absolute inset-0 bg-gradient-to-br from-transparent to-[#C08261]/3 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none rounded-3xl" />
                    <div className="text-3xl">{item.icon}</div>
                    <div className="space-y-1.5 z-10">
                      <h3 className="font-serif text-lg font-semibold text-stone-850">{item.label}</h3>
                      <p className="text-xs text-stone-500 font-sans leading-relaxed">{item.desc}</p>
                    </div>
                    <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-[#C08261] flex items-center space-x-1 z-10">
                      <span>Entrar</span>
                      <ChevronRight size={12} />
                    </span>
                  </button>
          ...
          {/* HUB DA COMUNIDADE — Gateway centralizado para sub-seções da Comunidade */}
          {activeSection === 'comunidade_hub' && !selectedDevotional && (
            <motion.div
              key="comunidade_hub"
              initial={{ opacity: 0, y: 15 }}
              animate={{ opacity: 1, y: 0 }}
            ...
            >
              <div className="space-y-2">
                <span className="text-[10px] font-mono uppercase tracking-widest text-[#C08261] font-bold">Hub da Comunidade</span>
                <h2 className="font-serif text-3xl md:text-4xl font-light text-stone-850 tracking-tight leading-tight">A <span className="text-[#C08261] font-semibold">Mesa de Família</span></h2>
                <p className="text-stone-500 text-sm max-w-xl font-sans leading-relaxed">
                  Cada cadeira a seguir é um lugar de comunhão real. Escolha onde o Espírito está chamando o seu coração agora.
                </p>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                {[
                  { id: 'primitiva_mural' as const,    icon: '🌱', label: 'O Mural Vivo',              desc: 'Necessidades, ofertas e cadeiras vazias da Igreja Primitiva. Amor prático em ação.', badge: `${activePrayersCount} ativos` },
                  { id: 'mural' as const,               icon: '🕊️', label: 'Mural de Oração & Fé',     desc: 'Junte-se em intercessão pelos pedidos da família do Despertar. Cada oração importa.', badge: null },
                  { id: 'testemunhas' as const,         icon: '🌟', label: 'Nuvem de Testemunhas',      desc: 'Histórias reais de fé que te lembram: o milagre sempre chega. Leia e fortaleça-se.', badge: null },
                  { id: 'profiles' as const,            icon: '🧭', label: 'Caminhos do Coração',       desc: 'Descubra sua identidade espiritual entre os 12 arquétipos de caminhada. Qual é a sua?', badge: null },
                  { id: 'primitiva_visao' as const,     icon: '📖', label: 'Igreja Primitiva & Visão',  desc: 'Nosso manifesto e como apoiar a infraestrutura do movimento com livros e doações.', badge: 'Apoiar' },
                  { id: 'primitiva_centelhas' as const, icon: '💡', label: 'Centelhas de Fé Ativa',     desc: 'Histórias e respostas co-criadas pela comunidade. Vote, escreva e faça circular a chama.', badge: 'Novo' },
                ].map(item => (
                  <button
                    key={item.id}
                    onClick={() => setActiveSection(item.id)}
                    className="group relative overflow-hidden bg-white border border-stone-200/60 rounded-3xl p-6 text-left hover:border-[#C08261]/40 hover:shadow-lg transition-all duration-300 flex flex-col space-y-4"
                  >
                    <div className="absolute inset-0 bg-gradient-to-br from-transparent to-[#C08261]/3 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none rounded-3xl" />
                    <div className="flex items-start justify-between">
                      <span className="text-2xl">{item.icon}</span>
                      {item.badge && (
                        <span className="text-[9px] font-mono font-bold uppercase tracking-wider bg-[#C08261]/15 text-[#C08261] px-2 py-0.5 rounded-full">{item.badge}</span>
                      )}
                    </div>
                    <div className="space-y-1.5 z-10">
                      <h3 className="font-serif text-base font-semibold text-stone-850">{item.label}</h3>
                      <p className="text-xs text-stone-500 font-sans leading-relaxed">{item.desc}</p>
                    </div>
                    <span className="text-[10px] font-mono font-bold uppercase tracking-wider text-[#C08261] flex items-center space-x-1 z-10">
                      <span>Entrar</span>
          ...
          {/* ACTIVE PORT: IGREJA PRIMITIVA — TAB MURAL VIVO (desagregado) */}
          {activeSection === 'primitiva_mural' && !selectedDevotional && (
            <motion.div
              key="primitiva_mural"
              initial={{ opacity: 0, y: 15 }}
              animate={{ opacity: 1, y: 0 }}
          ...
          {/* ACTIVE PORT: IGREJA PRIMITIVA — TAB VISÃO & APOIO (desagregado) */}
          {activeSection === 'primitiva_visao' && !selectedDevotional && (
            <motion.div
              key="primitiva_visao"
              initial={{ opacity: 0, y: 15 }}
              animate={{ opacity: 1, y: 0 }}
          ...
          {/* ACTIVE PORT: IGREJA PRIMITIVA — TAB CENTELHAS (desagregado) */}
          {activeSection === 'primitiva_centelhas' && !selectedDevotional && (
            <motion.div
              key="primitiva_centelhas"
              initial={{ opacity: 0, y: 15 }}
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: -15 }}
              className="space-y-4"
      ...
      {/* ── BOTTOM NAV BAR — mobile only ────────────────────────────────
          Visível apenas abaixo de md (768px). 5 destinos fixos.
          O menu lateral continua acessível em desktop (md:+) intacto.
      ─────────────────────────────────────────────────────────────── */}
      <nav
      ...
      >
        {([
          { id: 'home',    label: 'Início',      Icon: Home,     activeSections: ['home']     },
          { id: 'palavra_hub',   label: 'Palavra',     Icon: BookOpen, activeSections: ['palavra_hub','bible','devotionals','respiro','ebooks'] },
          { id: 'comunidade_hub',   label: 'Comunidade',  Icon: Users,    activeSections: ['comunidade_hub','mural','testemunhas','primitiva','primitiva_mural','primitiva_visao','primitiva_centelhas','profiles'] },
          { id: 'mesas',   label: 'Mesas',       Icon: Coffee,   activeSections: ['mesas']    },
          { id: 'profile', label: 'Perfil',      Icon: User,     activeSections: ['profile']  },
        ] as { id: typeof activeSection; label: string; Icon: React.ElementType; activeSections: typeof activeSection[] }[]).map(({ id, label, Icon, activeSections }) => {
          const active = activeSections.includes(activeSection as any);
          return (
            <button
              key={id}
              onClick={() => { setActiveSection(id); setSelectedDevotional(null); }}
            ...
            >
              {active && (
                <motion.div
                  layoutId="bottom_nav_dot"
                  className="absolute top-0 left-1/2 -translate-x-1/2 h-0.5 w-8 rounded-full bg-[#C08261]"
