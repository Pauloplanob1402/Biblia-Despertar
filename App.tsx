...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-stone-50 text-stone-900 flex flex-col md:flex-row antialiased selection:bg-[#C08261]/20">
      ...
      {/* ── Sub-nav PALAVRA — mobile only ── */}
      {(['bible','devotionals','respiro','ebooks'] as const).some(s => s === activeSection) && (
        <div className="md:hidden sticky top-[57px] z-30 bg-[#FAF8F5] border-b border-stone-200">
          <div className="tabs-scroll-wrapper">
            <div className="tabs-scroll flex px-4 gap-1 py-2">
              {([
                { id: 'devotionals', label: '☕ Devocional' },
                { id: 'bible',       label: '📖 Bíblia'     },
                { id: 'respiro',     label: '🌬️ Respiração'  },
                { id: 'ebooks',      label: '📚 Livros'      },
              ] as { id: typeof activeSection; label: string }[]).map(item => (
                <button
                  key={item.id}
                  onClick={() => { setActiveSection(item.id); setSelectedDevotional(null); }}
                  className={`whitespace-nowrap shrink-0 px-3 py-1.5 rounded-full text-xs font-mono font-bold uppercase tracking-wide transition ${
      ...
      {/* ── Sub-nav COMUNIDADE — mobile only ── */}
      {(['mural','testemunhas','primitiva','profiles'] as const).some(s => s === activeSection) && (
        <div className="md:hidden sticky top-[57px] z-30 bg-[#FAF8F5] border-b border-stone-200">
          <div className="tabs-scroll-wrapper">
            <div className="tabs-scroll flex px-4 gap-1 py-2">
              {([
                { id: 'mural',       label: '🙏 Orações'    },
                { id: 'testemunhas', label: '🌱 Testemunhos' },
                { id: 'primitiva',   label: '⚡ Igreja'      },
                { id: 'profiles',    label: '🧭 Jornadas'    },
              ] as { id: typeof activeSection; label: string }[]).map(item => (
                <button
                  key={item.id}
