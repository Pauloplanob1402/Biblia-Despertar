...
export default function App() {
  const [activeSection, setActiveSection] = useState<'home' | 'bible' | 'devotionals' | 'profiles' | 'mesas' | 'ebooks' | 'profile' | 'respiro' | 'testemunhas' | 'apps' | 'mural' | 'apoiar' | 'mural-vivo' | 'faisca'>('home');
  ...
  return (
    <div className="min-h-screen bg-stone-50 text-stone-900 flex flex-col md:flex-row antialiased selection:bg-[#C08261]/20">
      ...
      {/* MOBILE STICKY HEADER — top bar + contextual sub-tab chips */}
      <header className="md:hidden w-full bg-white border-b border-stone-200/55 sticky top-0 z-30 shrink-0 select-none">
        {/* Top row: brand + sementes */}
        <div className="flex items-center justify-between px-4 py-3">
          <div className="flex items-center space-x-2.5">
            ...
            <div className="text-left">
              ...
              <span className="text-[10px] font-mono tracking-widest text-[#C08261] uppercase leading-none block font-semibold">Mesa e Caminho</span>
            </div>
          </div>
          <div className="flex items-center gap-2">
            {/* Sementes pill */}
            <div className="flex items-center space-x-1 bg-[#f1f8f3] border border-emerald-100/75 px-2.5 py-1.5 rounded-full text-emerald-800 text-xs font-semibold font-mono shadow-xs">
            ...
            <button
              ...
              className="w-8 h-8 rounded-xl bg-orange-100/80 flex items-center justify-center text-base shrink-0 overflow-hidden"
            >
              {userProfile?.photoURL
                ? <img src={userProfile.photoURL} alt="Avatar" className="w-full h-full object-cover" />
                : <span>{userProfile?.avatarEmoji || '🕊️'}</span>}
            </button>
          </div>
        </div>

        {/* Contextual sub-tabs — aparecem apenas quando a seção pai está ativa */}
        ...
        {(['mural','testemunhas','mural-vivo','faisca','profiles'].includes(activeSection)) && (
          <div className="flex gap-2 px-4 pb-2.5 overflow-x-auto scrollbar-hide">
            {[
              { id: 'mural',      label: '🙏 Oração' },
              { id: 'faisca',     label: '✨ Faíscas' },
              { id: 'mural-vivo', label: '🔥 Mural Vivo' },
        ...
        {(activeSection === 'mesas') && (
          <div className="flex gap-2 px-4 pb-2.5 overflow-x-auto scrollbar-hide">
            {[
              ...
              <button
                key={item.sub}
                onClick={() => setMesasSubTab(item.sub)}
                className={`shrink-0 px-3 py-1.5 rounded-full text-xs font-semibold transition whitespace-nowrap ${
                  mesasSubTab === item.sub
                    ? 'bg-[#C08261] text-white shadow-sm'
                    : 'bg-stone-100 text-stone-600 hover:bg-stone-200'
                }`}
              >
        ...
        {(activeSection === 'apoiar') && (
          <div className="flex gap-2 px-4 pb-2.5 overflow-x-auto scrollbar-hide">
            <div className="shrink-0 px-3 py-1.5 rounded-full text-xs font-semibold bg-amber-500 text-white">
              📚 Livros & Apoio
            </div>
          </div>
        )}
      ...
      <aside className="hidden md:flex w-72 bg-white border-r border-stone-200/55 flex-col shrink-0 h-screen sticky top-0 z-30">
        ...
        <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
          ...
          {/* 3. COMUNIDADE — agrupa Mural + Testemunhas + Mural Vivo + Faíscas + Perfis */}
          <button
            id="nav-comunidade"
            onClick={() => { setActiveSection('mural'); setSelectedDevotional(null); }}
            className={`w-full flex items-center justify-between px-3 py-3 rounded-xl text-left tracking-wide text-sm font-semibold transition ${
              ['mural','testemunhas','mural-vivo','faisca','profiles'].includes(activeSection) ? 'bg-[#C08261]/10 text-[#C08261]' : 'text-stone-700 hover:bg-stone-50'
            }`}
          >
            <span className="flex items-center space-x-3">
              <Compass size={15} />
          ...
          {['mural','testemunhas','mural-vivo','faisca','profiles'].includes(activeSection) && (
            <div className="ml-7 space-y-0.5 pb-1">
              {[
                ...
                { id: 'faisca', label: 'Faíscas do Eterno', icon: <Sparkles size={12}/> },
                { id: 'mural-vivo', label: 'O Mural Vivo', icon: <Users size={12}/> },
      ...
      <main className="flex-1 overflow-y-auto px-4 py-6 md:px-12 md:py-12 max-w-7xl mx-auto w-full relative pb-24 md:pb-12">
        <AnimatePresence mode="wait">
          ...
          {activeSection === 'home' && !selectedDevotional && (
            ...
            >
              ...
              <div className="bg-[#FAF8F5] border-2 border-dashed border-[#C08261]/40 p-6 md:p-8 rounded-3xl flex flex-col md:flex-row justify-between items-start md:items-center gap-6 text-left shadow-xs">
                ...
                <button
                  onClick={() => setActiveSection('mural-vivo')}
          ...
          {activeSection === 'ebooks' && !selectedDevotional && (
            <motion.div
              ...
              className="space-y-2"
            >
              <LivrosDespertar currentUser={currentUser} userProfile={userProfile} />
              <EbookReader
                completedChapters={progress.completedChapters}
          ...
          {/* ACTIVE PORT: O MURAL VIVO */}
          {activeSection === 'mural-vivo' && !selectedDevotional && (
            <motion.div
              key="mural-vivo"
              initial={{ opacity: 0, y: 15 }}
              animate={{ opacity: 1, y: 0 }}
          ...
          {/* ACTIVE PORT: FAÍSCAS DO ETERNO */}
          {activeSection === 'faisca' && !selectedDevotional && (
            <motion.div
              key="faisca"
              initial={{ opacity: 0, y: 15 }}
              animate={{ opacity: 1, y: 0 }}
      ...
      >
        {([
          { id: 'home',    label: 'Início',      Icon: Home,     special: false },
          { id: 'bible',   label: 'Palavra',     Icon: BookOpen, special: false },
          { id: 'mural',   label: 'Comunidade',  Icon: Users,    special: false },
          { id: 'mesas',   label: 'Mesas',       Icon: Coffee,   special: false },
          { id: 'apoiar',  label: 'Livros',      Icon: BookOpen, special: true  },
        ] as { id: typeof activeSection; label: string; Icon: React.ElementType; special: boolean }[]).map(({ id, label, Icon, special }) => {
          const active = activeSection === id ||
            (id === 'mural' && ['mural-vivo','faisca','testemunhas','profiles'].includes(activeSection)) ||
            (id === 'bible' && ['devotionals','respiro','ebooks'].includes(activeSection));
          return (
            <button
              key={id}
              onClick={() => {
                if (id === 'bible' && !['devotionals','respiro','ebooks'].includes(activeSection)) {
            ...
            >
              {special ? (
                /* Apoiar — botão destaque central */
                <div className={`flex flex-col items-center gap-0.5 ${active ? 'opacity-100' : 'opacity-90'}`}>
                  <div className={`w-10 h-10 rounded-2xl flex items-center justify-center shadow-md transition-all ${
                    active ? 'bg-[#C08261] scale-110' : 'bg-stone-900'
                  }`}>
                    <Icon size={20} strokeWidth={2} color="white" />
                  </div>
                  <span style={{ fontSize: 10, lineHeight: 1.2, color: active ? '#C08261' : '#78716c', fontWeight: 700, whiteSpace: 'nowrap' }}>
                    {label}
                  </span>
                </div>
              ) : (
