...
interface IgrejaPrimitivaProps {
  ...
  /** Controlled active tab — injetado externamente pelo App.tsx ao desagregar sub-rotas */
  activeTab?: "comunhao" | "chamado" | "cocriacao";
  /** Callback para notificar o pai sobre mudanças de aba (quando não oculto) */
  onTabChange?: (tab: "comunhao" | "chamado" | "cocriacao") => void;
  /** Quando true, oculta o seletor de abas interno (controlado pelo pai via rota) */
...
export default function IgrejaPrimitiva({
  ...
  // Internal tab state (fallback quando não controlado externamente)
  const [internalActiveTab, setInternalActiveTab] = useState<"comunhao" | "chamado" | "cocriacao" | null>(
    null
  );

  // Hybrid state: usa o controlado externamente se disponível, senão o interno
  const activeTab = controlledActiveTab !== undefined ? controlledActiveTab : internalActiveTab;

  const setActiveTab = (tab: "comunhao" | "chamado" | "cocriacao" | null) => {
    setInternalActiveTab(tab as any);
    if (onTabChange && tab) onTabChange(tab as any);
  };
  ...
  return (
    ...
    >
      ...
      {/* NAVEGAÇÃO POR PÁGINAS — dois destinos distintos, navegação limpa */}
      {!hideTabsSelector && activeTab !== "comunhao" && activeTab !== "cocriacao" && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {/* Card: O Mural Vivo */}
          <button
            id="tab-mural-comunhao"
            onClick={() => setActiveTab("comunhao")}
            className="group relative overflow-hidden bg-[#FAF8F5] border border-stone-200/80 hover:border-[#C08261]/50 rounded-3xl p-6 text-left transition-all duration-300 hover:shadow-md cursor-pointer"
          >
            <div className="absolute top-0 right-0 w-32 h-32 bg-[#C08261]/5 rounded-full blur-2xl -mr-8 -mt-8 pointer-events-none group-hover:bg-[#C08261]/10 transition-all duration-500" />
            <div className="relative z-10 space-y-4">
              <div className="flex items-center justify-between">
                <div className="w-11 h-11 rounded-2xl bg-[#C08261]/10 flex items-center justify-center">
                  <Users size={20} className="text-[#C08261]" />
                </div>
                <span className="text-[10px] bg-stone-900 text-white px-2.5 py-1 rounded-full font-mono font-bold">
                  {muralItems.length} itens
                </span>
              </div>
              <div className="space-y-1.5">
                <h3 className="font-serif text-xl font-bold text-stone-850 group-hover:text-[#C08261] transition-colors">
                  O Mural Vivo
                </h3>
                <p className="text-stone-500 text-xs leading-relaxed font-sans">
                  Necessidades, ofertas, pedidos de oração e cadeiras vazias — a comunhão prática do Corpo de Cristo em tempo real.
                </p>
              </div>
              <div className="flex items-center gap-1.5 text-[#C08261] text-xs font-mono font-bold uppercase tracking-wider">
                <span>Entrar no Mural</span>
                <ArrowRight size={13} className="group-hover:translate-x-1 transition-transform" />
              </div>
            </div>
          </button>

          {/* Card: Faíscas do Eterno */}
          <button
            id="tab-cocriacao-pioneira"
            onClick={() => setActiveTab("cocriacao")}
            className="group relative overflow-hidden bg-stone-950 border border-stone-800 hover:border-amber-500/50 rounded-3xl p-6 text-left transition-all duration-300 hover:shadow-md cursor-pointer"
          >
            <div className="absolute top-0 right-0 w-40 h-40 bg-amber-500/10 rounded-full blur-2xl -mr-10 -mt-10 pointer-events-none group-hover:bg-amber-500/15 transition-all duration-500" />
            <div className="relative z-10 space-y-4">
              <div className="flex items-center justify-between">
                <div className="w-11 h-11 rounded-2xl bg-amber-500/15 flex items-center justify-center">
                  <Sparkles size={20} className="text-amber-400" />
                </div>
                <span className="text-[10px] bg-amber-500 text-white px-2.5 py-1 rounded-full font-mono font-bold uppercase tracking-wider">
                  Fé Ativa
                </span>
              </div>
              <div className="space-y-1.5">
              ...
              <div className="flex items-center gap-1.5 text-amber-400 text-xs font-mono font-bold uppercase tracking-wider">
                <span>Acender Minha Faísca</span>
                <ArrowRight size={13} className="group-hover:translate-x-1 transition-transform" />
              </div>
            </div>
          </button>
      ...
      {/* BREADCRUMB DE VOLTA — quando uma página está ativa */}
      {!hideTabsSelector && (activeTab === "comunhao" || activeTab === "cocriacao") && (
        <div className="flex items-center gap-3">
          <button
            onClick={() => setActiveTab(undefined as any)}
            className="flex items-center gap-1.5 text-stone-400 hover:text-stone-700 text-xs font-mono uppercase tracking-wider transition group"
          >
          ...
          <span className="text-xs font-mono uppercase tracking-wider font-bold text-stone-700">
            {activeTab === "comunhao" ? "O Mural Vivo" : "✨ Faíscas do Eterno"}
          </span>
        </div>
      )}
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="space-y-1">
                  <h3 className="font-serif text-lg md:text-xl font-bold text-stone-850">
                    Faíscas do Eterno: Frutos de Edificação e Testemunho
                  </h3>
